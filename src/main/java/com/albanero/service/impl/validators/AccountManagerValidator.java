...
@Component
public class AccountManagerValidator implements Validator {
    @Override
    public void doValidate(CustomerRequest customerRequest) {
        if (StringUtils.isBlank(customerRequest.getAccountManager())) {

            throw new ValidationException(ErrorCode.ACCOUNT_MANAGER_VALIDATION_FAILED.getErrorMessage(),
